...
const Filter = () => {
  ...
  const isUnique = (index, rooms) => {
    return sort([...new Set(rooms.map((room) => room[index]))]);
  };
  ...
  const myType = ['all', ...isUnique('type', rooms)];
  const myCapacity = ['any', ...isUnique('capacity', rooms)];
